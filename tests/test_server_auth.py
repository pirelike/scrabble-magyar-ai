...
class TestGameMovesAPI:
    def _login(self, client):
        import auth
        _, user_id = auth.create_user('moves@test.com', 'MovesUser', 'pass123')
        token = auth.create_session(user_id)
        client.set_cookie('session_token', token, domain='localhost')
        return user_id
    ...
    def test_get_moves_success(self, client):
        ...
        game_id = auth.save_game('room-1', 'Room', '{}', False,
                                 [{'player_name': 'Alice', 'user_id': user_id, 'score': 0}])
    ...
    def test_get_moves_no_auth(self, client):
        res = client.get('/api/game/1/moves')
        assert res.status_code == 401

    def test_get_moves_not_participant(self, client):
        import auth
        self._login(client)
        _, other_id = auth.create_user('other@test.com', 'Other', 'pass123')
        game_id = auth.save_game('room-2', 'Room', '{}', False,
                                 [{'player_name': 'Other', 'user_id': other_id, 'score': 0}])
        res = client.get(f'/api/game/{game_id}/moves')
...
class TestAbandonGameAPI:
    def test_abandon_success(self, client):
        ...
        _, user_id = auth.create_user('test@example.com', 'TestUser', 'pass123')
        players = [{'player_name': 'TestUser', 'user_id': user_id, 'score': 50}]
        game_id = auth.save_game('room-1', 'Room', '{}', False, players_data=players, owner_name='TestUser')
        token = auth.create_session(user_id)
        client.set_cookie('session_token', token, domain='localhost')
