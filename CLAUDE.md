...
| `save_game` | kliens→szerver | Manuális mentés (owner only) |
| `restore_game` | kliens→szerver | Mentett játék visszaállítása (várakozó szoba létrehozás) |
...
### Adatbázis táblák
- **`saved_games`**: id, room_id, room_name, state_json, status ('active'/'finished'/'abandoned'), challenge_mode, owner_name, created_at, updated_at
...
### Manuális mentés (owner-only)
- Nincs autosave — mentés kizárólag a szoba tulajdonosának kilépés menüjéből ("Mentés és kilépés")
- `save_game` Socket.IO event → `_save_game_to_db(room_id)` — rate limit: 3/30mp
- `to_save_dict()` / `from_save_dict()` szerializáció a `Game` osztályban
- Lépés-naplózás: `_record_move()` board snapshot-tal
...
- Nem csatlakozott játékosok zsetonjai visszakerülnek a zsákba
...
- **Lobby-first restore**: `restore_game` event → privát várakozó szoba az eredeti játékosoknak
- **Kilépés menü**: owner 3 gomb (mentés+kilépés / kilépés / mégsem), nem-owner 2 gomb (kilépés / mégsem)
- **Megerősítő dialog**: `confirm()` helyett saját UI dialog (mentett játék törlésénél)
- **DB migráció**: `owner_name` oszlop a `saved_games` táblához

### Játék mentés, visszajátszás, kilépés gomb, profil oldal (2026-02-27)
- **Visszajátszás**: Befejezett játékok lépésenkénti visszanézése board snapshot-okkal
- **Profil oldal**: Statisztikák (4 kártya) + játékelőzmények lista + visszajátszás gomb
- **DB séma**: `saved_games`, `game_players`, `game_moves` táblák + persistence funkciók
